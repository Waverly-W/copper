...
function createHistoryItemId(now = Date.now()) {
  historyItemSequence = (historyItemSequence + 1) % 100000;
  return `history-${now}-${historyItemSequence}`;
}
...
export function createImageHistoryItem(imageAsset, now = Date.now()) {
  const title =
    imageAsset.imagePath?.split(/[\\/]/).pop() || `image-${now}.png`;
...
export function mergeImageHistoryItem(item, imageAsset, now = Date.now()) {
  const title =
    imageAsset.imagePath?.split(/[\\/]/).pop() ||
    item.title ||
    `image-${now}.png`;
...
export function decorateHistoryItem(item) {
  ...
  const lastCopiedAt =
    item.lastCopiedAt || item.updatedAt || item.createdAt || Date.now();
  const textMetrics =
    item.type === "image"
      ? {
      ...
      : item.textMetrics ||
        getTextMetrics(item.contentText || item.title || "");
...
export function isSameTextHistoryItem(item, text) {
  ...
  return (
    normalizeText(item.contentText || item.title || "") === normalizeText(text)
  );
}
...
export function isSameImageHistoryItem(item, imageAsset) {
  ...
  return (
    ...
    String(imageAsset.imagePath || "").trim()
  );
}
